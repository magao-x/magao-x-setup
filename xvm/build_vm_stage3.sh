...
if [[ -e ./output/xvm_stage3.qcow2 ]]; then
    echo "Stage 3 image populated from cache. Skipping stage 3."
    exit 0
fi
if [[ -e ./output/xvm_stage2.qcow2 ]]; then
    if [[ -z $CI ]]; then
        ...
        mv ./output/xvm_stage2.qcow2 ./output/xvm.qcow2
    fi
...
ssh -p $guestPort -o "UserKnownHostsFile /dev/null" -o "StrictHostKeyChecking=no" -i ./output/xvm_key xsup@localhost 'bash -s' < ./guest_provision_up_to_build.sh || exit 1

...
export CI=1
sudo bash -lx ~/magao-x-setup/steps/ensure_dirs_and_perms.sh || (echo 'Failed to create dirs' && exit 1)
sudo bash -lx ~/magao-x-setup/install_third_party_deps.sh || (echo 'Failed to install 3rd party deps' && exit 1)
